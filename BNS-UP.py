...
def packet_frame(raw_data):
    dest_mac,srs_mac,protocol = struct.unpack("! 6s 6s H",raw_data[:14])
        ...
        if eth_proto == 8:
            ...
            if proto == 1 or proto == 2:
                ...
                print(f'''\t\t Type: {icmp_type}, Code: {code}, Check Sum:{check_sum},
                      \t\tdata: {formate_actual_packet_data(tab_2,icmp_data)}\n''')

            # protocolo = 6 means it's a TCP packet
            elif proto == 6:
                tdp_src_port, tdp_dest_port, sequence, acknowledgement, flag_urg, flag_ack, flag_psh, flag_rst, flag_syn, flag_fin, tcp_data = tcp_packets_unpacking(ipv4_data)
                print('\tTDP Packet:')
                print(f'''\t\t Source Port No.: {tdp_src_port}, Destionation Port No.: {tdp_dest_port},
                      ...
                      \t\tdata: {formate_actual_packet_data(tab_2,tcp_data)}\n''')
            ...
            # protocolo = 17 means it's an UDP packet
                ...
                print(f'''\t\t Source Port No.: {udp_src_port}, Destionation Port No.: {udp_dest_port}, Size:{size},
                      \t\tdata: {formate_actual_packet_data(tab_2,udp_data)}\n''')
            else:
                print(f'''\tdata: {formate_actual_packet_data(tab_2,ipv4_data)}\n''')
...
def formate_actual_packet_data(prefix,string,size=80):
    size -= len(prefix)
